    ...
    def _save_file(file, folder_name):
        ...
        if file and UserProfileController._allowed_file(file.filename):
            ...
            if not os.path.exists(file_path):
                print(f"[ERROR] File save failed: {file_path}")
                return None
            
            print(f"[FILE SAVED] Successfully saved to: {file_path}")
            
            # Normalize path to use forward slashes for URL compatibility
            # This ensures the path works correctly in URLs on all platforms
            normalized_path = file_path.replace('\\', '/')
            print(f"[FILE SAVED] Normalized path for DB: {normalized_path}")
